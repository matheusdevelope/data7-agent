...
function Home() {
  ...
  async function GetLocalPass() {
    const result = await window.ElectronAPI.GetLocalPassApp();
    if (typeof result == 'string') return setLocalPass(result);
    if (!result) return setLocalPass(false);
    return setDialogState({
      ...DefaultDialog,
      isOpen: true,
      message: 'Houve um erro ao buscar a senha local!\nError: ' + JSON.stringify(result),
    });
  }
  async function SetLocalPass(pass: string) {
    const result = await window.ElectronAPI.SetLocalPassApp(pass);
    if (result === true)
      return setDialogState({
        ...DefaultDialog,
        isOpen: true,
    ...
    return setDialogState({
      ...
      isOpen: true,
      onClickOK: CloseDialog,
      message: 'Houve um erro ao salvar a senha local!\nError: ' + JSON.stringify(result),
    });
  }
  ...
  return (
    <Container>
      <Form onSubmit={OnSubmitForm}>
        {!localPass ? <Text>Crie uma senha de acesso:</Text> : <Text>Insira a sua senha:</Text>}
        <Input
          type={'password'}
          value={pass}
          onChange={(e) => {
          ...
          ref={passRef}
        />
        <Button type="submit" style={{ backgroundColor: pass.length < 8 ? '#ddd' : '#348ccb' }}>
